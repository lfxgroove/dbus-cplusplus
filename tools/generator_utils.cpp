...
void _parse_signature_ty(const string &signature, vector<Ty> &types, unsigned int &i)
{
	for (; i < signature.length(); ++i)
	{
		switch (signature[i])
					...
					case '{':
					{
						...
						if (!atom)
						{
							...
							exit(-1);
                                                ...
                                                type.tplParams.push_back(Ty{atom});
						++i;
						break;
					}
					default:
					{
                                                type.ident = "std::vector";
						break;
					}
				}
                                vector<Ty> parsedTypes{};
				_parse_signature_ty(signature, parsedTypes, i);
                                for (Ty parsedType : parsedTypes)
                                {
                                        type.tplParams.push_back(parsedType);
                                }
...
Ty signature_to_ty(const string &signature)
{
        unsigned int i = 0;
        vector<Ty> types;
        _parse_signature_ty(signature, types, i);
        assert(types.size() == 1);
        return types[0];
}
